...
3. 如需中文总结，再填：
   ...
   - `ARK_PLANNER_TIMEOUT_SECONDS` / `ARK_WRITER_TIMEOUT_SECONDS`（可选，默认 20 / 45）
...
为控制 LLM 上下文长度，系统会先压缩输入：每个来源保留高权重证据，最新内容优先，最近 3 天 raw 作为滚动上下文，微信群归档先过滤噪音。监控源少于 20 个时，Planner 最多保留 18 条高权重记录，尽量覆盖不同博主/来源。LLM 分两层：Planner 只做目录、核心点、论点论据和行动框架，Writer 用更快模型完成最终报告表达；报告结构和 PDF 排版由代码固定。
